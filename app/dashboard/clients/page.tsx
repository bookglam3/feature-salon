        ...
        (appts || []).forEach((apt) => {
          ...
          if (!clientMap.has(key)) {
            clientMap.set(key, {
              ...
              totalBookings: 1,
              totalSpent: apt.services?.price || 0,
            });
          } else {
            const client = clientMap.get(key);
            client.totalBookings += 1;
            client.totalSpent += apt.services?.price || 0;
          }
        });
                  ...
                  clientHistory.map((b) => (
                    <div key={b.id} style={{ padding: "14px 20px", borderBottom: "0.5px solid #F1F5F9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "4px" }}>
                        <span style={{ fontSize: "13px", fontWeight: 500, color: "#0F172A" }}>£{b.services?.price || "—"}</span>
                        <span style={{ background: b.status === "confirmed" ? "#ECFDF5" : b.status === "cancelled" ? "#FEF2F2" : "#FFF7ED", color: b.status === "confirmed" ? "#166534" : b.status === "cancelled" ? "#DC2626" : "#92400E", fontSize: "10px", padding: "2px 8px", borderRadius: "20px" }}>
                          {b.status}
                        </span>
                      </div>
                    </div>
